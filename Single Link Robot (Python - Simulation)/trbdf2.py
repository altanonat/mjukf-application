...
def trbdf2(state,stateDot,time,k,Jm,BR,Ktau,Jl,m,g,h,gammaIvp,deltat,nos):
    statek    = state+gammaIvp*deltat*stateDot;
    fkgamma   = mathmodel(statek,stateDot,time,k,Jm,BR,Ktau,Jl,m,g,h,nos);
    xkgamma   = state+gammaIvp*(deltat/2)*(stateDot+fkgamma);
    time      = time+deltat;
    
    state1    = state+deltat*stateDot;
    stateDot1 = mathmodel(state1,stateDot,time,k,Jm,BR,Ktau,Jl,m,g,h,nos);
