...
def plotVars(tPlot,statePlot,stateDotPlot,thetaPlot,meas,thetaReal,ni):
    fig1 = plt.figure(1)
    ...
    fig3=plt.figure(3)
    line1, = plt.plot(tPlot[:], thetaPlot[0,:],linestyle='solid', linewidth=0.5, color='gray')
    line2, = plt.plot(tPlot[:], thetaReal[0, :],linestyle='-.', linewidth=1, color='black')
    plt.legend([line1, line2], ['$\\hat{\\theta}$', 'Real Value'])
    plt.grid(linewidth=0.2)
    plt.title('$k_{flow}$ Estimate - (MJUKF)', fontdict=font)
    plt.xlabel('Time (s)', fontdict=font)
    plt.tick_params(axis='x', labelsize=11)
    plt.tick_params(axis='y', labelsize=11)
    ...
    fig3.savefig('Svgs/kflow-mjukf.svg')
    plt.show()
