...
def trbdf2(state,stateDot,time,qi,c1,A1,c2,A2,gammaIvp,deltat,nse,thetaCap,stateDotk,stateDot1):
    statek    = state+gammaIvp*deltat*stateDot;
    stateDotk = mathmodel(statek,stateDotk,time,qi,c1,A1,c2,A2,nse,thetaCap);
    fkgamma   = (stateDotk+stateDot)/2;
    xkgamma   = state+gammaIvp*(deltat/2)*(stateDot+fkgamma);
    time      = time+deltat;
    
    state1    = state+deltat*stateDot;
    stateDot1 = mathmodel(state1,stateDot1,time,qi,c1,A1,c2,A2,nse,thetaCap);
