...
def plotVars(tPlot,statePlot,stateDotPlot,thetaCapPlot,meas,thetaReal):
    fig1 = plt.figure(1)
    ...
    plt.grid(linewidth=0.2)
    plt.title('$\\omega_{m}$ (rad/s) - (MJUKF)', fontdict=font)
    plt.xlabel('Time (s)', fontdict=font)
    plt.tick_params(axis='x', labelsize=11)
    plt.tick_params(axis='y', labelsize=11)
    plt.ylabel('Angular velocity of the motor - $\\omega_{m}$ (rad/s)', fontdict=font)
    fig2.savefig('Svgs/omegam-mjukf.svg')
    plt.show()
    ...
    plt.grid(linewidth=0.2)
    plt.title('$\\theta_{l}$ (rad) - (MJUKF)', fontdict=font)
    plt.xlabel('Time (s)', fontdict=font)
    plt.tick_params(axis='x', labelsize=11)
    plt.tick_params(axis='y', labelsize=11)
    plt.ylabel('Angular position of the link - $\\theta_{l}$ (rad)', fontdict=font)
    fig3.savefig('Svgs/thetal-mjukf.svg')
    plt.show()
    ...
    plt.grid(linewidth=0.2)
    plt.title('$\\omega_{l}$ (rad/s) - (MJUKF)', fontdict=font)
    plt.xlabel('Time (s)', fontdict=font)
    plt.tick_params(axis='x', labelsize=11)
    plt.tick_params(axis='y', labelsize=11)
    plt.ylabel('Angular velocity of the link - $\\omega_{l}$ (rad/s)', fontdict=font)
    fig4.savefig('Svgs/omegal-mjukf.svg')
    plt.show()
    
    fig5=plt.figure(5)
    line1, = plt.plot(tPlot[:], thetaCapPlot[0,:],linestyle='solid', linewidth=0.5, color='gray')
    line2, = plt.plot(tPlot[:], thetaReal[0, :],linestyle='-.', linewidth=1, color='black')
    plt.legend([line1, line2], ['$\\hat{\\theta}_{1}$', 'Real Value'])
    plt.grid(linewidth=0.2)
    plt.title('$K_{\\tau}$ Estimate - (MJUKF)', fontdict=font)
    plt.xlabel('Time (s)', fontdict=font)
    plt.tick_params(axis='x', labelsize=11)
    plt.tick_params(axis='y', labelsize=11)
    ...
    fig5.savefig('Svgs/ktau-mjukf.svg')
    plt.show()
    
    fig6=plt.figure(6)
    line1, = plt.plot(tPlot[:], thetaCapPlot[1,:],linestyle='solid', linewidth=0.5, color='gray')
    line2, = plt.plot(tPlot[:], thetaReal[1, :],linestyle='-.', linewidth=1, color='black')
    plt.legend([line1, line2], ['$\\hat{\\theta}_{2}$', 'Real Value'])
    plt.grid(linewidth=0.2)
    plt.title('$m$ Estimate - (MJUKF)', fontdict=font)
    plt.xlabel('Time (s)', fontdict=font)
    plt.tick_params(axis='x', labelsize=11)
    plt.tick_params(axis='y', labelsize=11)
    # Set y-axis limits to 0 and 10
    # plt.ylim(0, 0.4)  # (ymin, ymax)
    plt.ylabel('Parameter Estimate ($m$)', fontdict=font)  
    fig6.savefig('Svgs/m-mjukf.svg')
    plt.show()
